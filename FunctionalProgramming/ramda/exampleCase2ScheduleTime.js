...
/** Ramda (@link https://ramdajs.com/docs/#partition) */
const isNotRented = (property) => R.prop('status', property) !== constants.PROPERTY_STATUS.RENTED;
const [ listingProperties, rentedProperties ] = R.partition(isNotRented, propertyList);
...
const mapStructureAvailable = ({ label, id }) => ({ label, id });
...
// 1. iterate per all schedules keys ['YYYY-MM-DD', ex: '2017-08-06']
const _filterAvailableDatesWithRamda = (schedules) => {
  let dates = [];
...
// 2. map times to new structure { label, id, available }
const MapIndexed = R.addIndex(R.map);
...
// 4. map to "new" structure again { label, id }
const mapStructureAvailable = ({ label, id }) => ({ label, id });
...
// 5. create "dates" with structure { date, times }
const _filterAvailableDatesWithRamda = (schedules) => {
  let dates = [];
...
// 6. sorts dates array by date field
const sortByDate = R.sortBy(R.prop('date'));
...
// 7. filters only dates with not empty times
const isNotEmptyTime = (element) => element.times.length > 0;
...
// 8. slice per MAX_AVAILABLE_DATES threshold (now: 3)
const mapValidScheduleDates = R.pipe(
  sortByDate,
  R.filter(isNotEmptyTime),
  R.slice(0, MAX_AVAILABLE_DATES)
);
